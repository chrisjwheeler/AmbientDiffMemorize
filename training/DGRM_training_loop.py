...
def training_loop(
    ...
    added_noise_var     = 0,        # Amount of Gaussian noise during early part of the training.
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                nature_noise = dataset_item['noise'].to(device) # Need to add nature noise to the image, dataset
                loss = loss_fn(net=ddp, images=images, labels=labels, nature_noise=nature_noise,
                                current_sigma=cur_sigma, augment_pipe=augment_pipe)
        ...
        ema_beta = 0.5 ** (batch_size / max(ema_halflife_nimg, 1e-8)) # equivalent to mode1
        ...
        if (state_dump_ticks is not None) and (done or cur_tick % state_dump_ticks == 0) and dist.get_rank() == 0 and cur_tick != 0:
        ...
        if dist.get_rank() == 0:
            ...
            wandb.log({"noise_variance": cur_sigma}, step=cur_tick * snapshot_ticks)
            stats_jsonl.flush()
        dist.update_progress(cur_nimg // 1000, total_kimg)
