...
@click.option('--loss_type',          help='Loss function', metavar='edm|ambient_highnoise_edm_lownoise',               type=click.Choice(['edm', 'ambient_highnoise_edm_lownoise']), default='ambient_highnoise_edm_lownoise', show_default=True)
@click.option('--nature_noise_fixed',          help='Whether to fix the nature noise or not', metavar='BOOL',       type=bool, default=False, show_default=True)

# Noise level above that noise level, we will use noisy target
...
@click.option('--seed',          help='Random seed  [default: random]', metavar='INT',              type=int, default=7)
...
def main(**kwargs):
    ...
    if dist.get_rank() == 0:
        wandb.init(project="memorization", 
    ...
    c.dataset_kwargs = dnnlib.EasyDict(path=opts.data, use_labels=opts.cond, xflip=opts.xflip, cache=opts.cache, sigma=opts.sigma,
                                       corruption_probability_per_image=1.0, corruption_probability_per_pixel=1.0,
                                       only_positive=False, nature_noise_fixed=opts.nature_noise_fixed)
    
    c.added_noise_var = opts.sigma
    ...
    if opts.optimizer == "adam":
        c.optimizer_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=opts.lr, 
                                             betas=[0.9,0.999], eps=1e-8, weight_decay=opts.weight_decay)
    ...
    elif opts.loss_type == 'ambient_highnoise_edm_lownoise':
        c.loss_kwargs.class_name = 'training.loss.AmbientHighNoise_EDMLowNoise_Loss'
